        ...
        <div class="bg-white rounded-xl shadow p-4 mb-6">
            <form method="GET" class="flex flex-wrap gap-4 items-end">
                ...
                <div>
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" class="border rounded px-3 py-2" placeholder="Search description...">
                </div>
                <div>
                ...
                <a href="{{ route('admin.audit-logs') }}" class="text-gray-500 hover:text-gray-700 px-3 py-2">Clear</a>
            </form>
        </div>

        <div class="bg-white rounded-xl shadow overflow-hidden">
            <table class="w-full text-sm">
                <thead><tr class="bg-gray-50 text-left"><th class="p-4">Time</th><th class="p-4">User</th><th class="p-4">Module</th><th class="p-4">Action</th><th class="p-4">Description</th><th class="p-4">IP</th></tr></thead>
                <tbody>
                    @forelse ($logs as $log)
                        <tr class="border-t hover:bg-gray-50">
                            <td class="p-4 text-xs">{{ $log->created_at->format('M d, Y h:i A') }}</td>
                            <td class="p-4">{{ $log->user->name ?? 'System' }}</td>
                            <td class="p-4"><span class="bg-gray-100 px-2 py-1 rounded text-xs">{{ $log->module }}</span></td>
                            ...
                            <td class="p-4 text-xs text-gray-400">{{ $log->ip_address ?? '-' }}</td>
                        </tr>
                    @empty
                        <tr><td colspan="6" class="p-8 text-center text-gray-400">No audit logs found.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
