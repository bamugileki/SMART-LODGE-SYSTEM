            ...
            <div class="flex items-center space-x-3">
                @auth
                    @if (Auth::user()->isAdmin())
                        ...
                        <a href="{{ route('admin.quick-links.index') }}" class="hidden md:inline-flex items-center px-3 py-2 text-sm font-medium text-gray-700 hover:text-indigo-600 transition-colors duration-200 relative after:absolute after:bottom-0 after:left-3 after:right-3 after:h-0.5 after:bg-indigo-600 after:scale-x-0 after:transition-transform after:duration-300 hover:after:scale-x-100">Quick Links</a>
                    @endif
        ...
        <div x-show="open" 
            ...
            @auth
                @if (Auth::user()->isAdmin())
                    ...
                    <a href="{{ route('admin.quick-links.index') }}" @click="open = false" class="block px-3 py-2 text-gray-700 hover:bg-indigo-50 hover:text-indigo-600 rounded-lg text-sm transition-all duration-200 hover:translate-x-1">Quick Links</a>
                @endif
                @if (Auth::user()->isManager())
